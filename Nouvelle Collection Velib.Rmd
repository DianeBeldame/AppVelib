...
```{r}
files <- c("synop.201607.csv","synop.201608.csv","synop.201609.csv","synop.201610.csv","synop.201611.csv","synop.201612.csv")
...
### on associe la météo aux données historiques des stations
...
for(i in 1:nrow(my.data.stations)){
  ...
  x       <- x[-which(names(x)=="loc")] # pas necessaire
  ...
  output  <- m$update(query = paste0('{"number": ',my.data.stations$number[i],'}'),update = paste0('{"$set": {"loc.type" : "Point", "loc.coordinates" : [',my.data.stations[i,]$position$lng,',',my.data.stations[i,]$position$lat,']}}'))
  cat("\r",i,"/",nrow(my.data.stations))

}

# for(i in 950:nrow(my.data.stations)){
#     output  <- m$update(query = paste0('{"number": ',my.data.stations$number[i],'}'),update = paste0('{"$set": {"loc.type" : "Point", "loc.coordinates" : [',my.data.stations[i,]$position$lng,',',my.data.stations[i,]$position$lat,']}}'))
#     cat("\r",i," / ",nrow(my.data.stations))
# 
# }
