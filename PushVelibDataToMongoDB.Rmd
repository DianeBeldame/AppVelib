...
tab <- m$aggregate(paste0('[
  ...
  { "$project" : {"_id": 0, "serie.last_update" : 1, "serie.available_bikes":1 , "serie.available_bikes_stands":1,"number": 1} }
  ]')) 


# J'ai un problème de conversion de date: si vous pouviez le corriger...
library(ggplot2)
ggplot(tab$serie)+aes(x=as.POSIXct(last_update/1000,origin="1970-01-01", tz="GMT"),y=available_bikes)+geom_line()+labs(title=paste0('station ',tab$number[1]),x="date",y="available bikes")+scale_x_datetime(date_labels = "%c")+geom_line(aes(x=as.POSIXct(tab$serie$last_update/1000,origin="1970-01-01", tz="GMT"),y=tab$serie$number))
